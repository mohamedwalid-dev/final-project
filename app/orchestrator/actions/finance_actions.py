...
def _get_db():
    """Return a proxy over the Node API client for Finance operations."""
    from core.node_api_client import get_node_api_client
    
    class NodeFinanceDBProxy:
        def __init__(self):
            self.client = get_node_api_client()
            
        async def get_customer_email(self, customer_id: str):
            try:
                cust = await self.client.get_customer(customer_id)
                return cust.get("email") if cust else None
            except Exception:
                return None
                
        async def update_invoice_status(self, invoice_id: str, status: str):
            await self.client.update_resource(f"/finance/invoices/{invoice_id}", {"status": status}, method="PATCH")
            
        async def update_customer_status(self, customer_id: str, **kwargs):
            await self.client.update_resource(f"/finance/customers/{customer_id}", kwargs, method="PATCH")
            
        async def log_collection_action(self, **kwargs):
            await self.client.create_resource("/finance/collections/log", kwargs)
            
        async def get_escalation_status(self, invoice_id: str):
            try:
                return await self.client.get_escalation_status(invoice_id)
            except Exception as e:
                return {"error": str(e)}

        async def create_legal_case(self, **kwargs):
            return await self.client.create_resource("/finance/legal-cases", kwargs)
        ...
        def invoices(self):
            class InvoicesProxy:
                ...
                async def update_one(self, query, update):
                    inv_id = str(query["_id"])
                    updates = update.get("$set", {})
                    await self.parent.client.update_resource(f"/finance/invoices/{inv_id}", updates, method="PATCH")
            return InvoicesProxy(self)

    return NodeFinanceDBProxy()
