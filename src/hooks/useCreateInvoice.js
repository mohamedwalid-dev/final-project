...
// ── Constants ─────────────────────────────────────────────────────────────────
export const STEPS = [
  { id: "billing",      label: "Billing Info",  icon: "billing" },
  { id: "line-items",   label: "Line Items",    icon: "line-items" },
  { id: "calculations", label: "Calculations",  icon: "calculations" },
  { id: "finalize",     label: "Finalize",      icon: "finalize" },
];
...
function validateStep(step, form, lineItems) {
  ...
  if (!form.status) {
    errs.status = "Invoice status is required";
  } else if (!allowedStatuses.includes(form.status)) {
    errs.status = "Invalid invoice status";
  }
